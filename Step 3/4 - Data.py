...
def retrieveAntennaData(AntennaToMatch, Freq, FilePath): # 3000 'Retrieve the antenna data
...
def retrieveWaveguideCableAttenuation(Freq, TransLine, FilePath):
...
def calculateFreeSpaceLossAtmosphericAttenuation(PathIndex, Freq, FilePath):
...
def retativeDispersionFactor(PathIndex, FilePath1, FilePath2):
...
FilePath = ExampleS3FolderPath + "/Data/PathDataPass1.csv"
path_data_pass_df_1 = pd.read_csv(FilePath)

# Get local output file path from pyqt interface
FilePath = ExampleS3FolderPath + "/Data/PathDataPass2.csv"
path_data_pass_df_2 = open(FilePath, "w+")

# Get local RadioFilesRev1.csv file path from pyqt interface 
RadioFolderPath = ex.openFolderNameDialog("Find Radios Folder")
RadioFilePath = RadioFolderPath + "/RadioFilesRev1.csv"

# Get local AntennaFilesRev1.csv file path from pyqt interface 
AntennaFolderPath = ex.openFolderNameDialog("Find Antennas Folder")
AntennaFilePath = AntennaFolderPath + "/AntennaFilesRev1.csv"

# Get local WaveguideRev1.csv file path from pyqt interface 
WaveGuideFilePath = AntennaFolderPath + "/WaveguideRev1.csv"

# Get local PathParameters.csv file path from pyqt interface 
PathParametersFilePath = ExampleS3FolderPath + "/PathParameters.csv"

# Get local AveragePaths.csv file path from pyqt interface 
AveragePathFilePath = ExampleS3FolderPath + "/AveragePaths.csv"
...
for index, row in path_data_pass_df_1.iterrows():
    ...
    temp = retrieveAntennaData(AntennaToMatch, Freq, AntennaFilePath) # 'find antenna parameters
    if temp:
    ...
    if LeftSDAntenna:
        ...
        temp = retrieveAntennaData(AntennaToMatch, Freq, AntennaFilePath)
        if temp:
            AntennaGain, AntennaModel = temp[0], temp[1]
    ...
    if TransLine:
        LineAttn = retrieveWaveguideCableAttenuation(Freq, TransLine, WaveGuideFilePath)
        if LineAttn:
            if TransLine:
    ...
    temp = calculateFreeSpaceLossAtmosphericAttenuation(PathIndex, Freq, PathParametersFilePath)
    if temp:
    ...
    RelDispFactor = retativeDispersionFactor(PathIndex, AveragePathFilePath, PathParametersFilePath)
    if RelDispFactor:
...
#if __name__ == '__main__':  # pyqt exit
   # sys.exit(app.exec_())