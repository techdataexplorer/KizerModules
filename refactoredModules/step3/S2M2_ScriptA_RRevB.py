...
class ScriptA(object):
    ...
    def setTerrainOption(self, selectedOption):
        ...
        option = int(option)
        if(option == 1):
            self.TheMainOption = 12
            self.TheOption = 12
        elif(option == 2):
            self.TheMainOption = 5
            self.TheOption = 5
        elif(option == 3):
            self.TheMainOption = 6
            self.TheOption = 6
        elif(option == 4):
            self.TheMainOption = 8
            self.TheOption = 8
        elif(option == 5):
            self.TheMainOption = 7
            self.TheOption = 7
        elif(option == 6):
            self.TheMainOption = 4
            self.TheOption = 4
        elif(option == 7):
            self.TheMainOption = 10
            self.TheOption = 10
        elif(option == 8):
            self.TheMainOption = 11
            self.TheOption = 11
    ...
    def execute(self):
        ...
        criteria_df = pd.read_csv(Path(CriteriaFilePath), header=None)
        ...
        if self.FeetMeters != "F" and self.FeetMeters != "M":
            print("Fourth line of <Criteria.csv> not understood.")
        ...
        TheDataFilePath = self.ExampleS2AFolderPath + "/" + "Paths" + ".csv" # TheDataFile comes from 9000
        print("Test File: " + TheDataFilePath)
        df = pd.read_csv(TheDataFilePath)      # data_df == #10 in the basic module
        ...
        for index, row in df.iterrows():
            ...
            if TowerData == 0:
                ...
                self.Latitude2 = row[4]
                self.Longitude2 = row[5]
                #self.Latitude1, self.Latitude1, self.Latitude1, self.Latitude1 = row['Latitude1'], row['Latitude2'], row['Longitude1'], row['Longitude2']
            ...
            if COUNTER == 1 and importPathFlag:
                for i in range(self.MaxGlobal):
                    if self.GlobalTerrain[i] != "":
                        dataFileOutput.append(str(self.GlobalTerrain[i]))
                        DataFile = str(self.TerrainFolderPath) + "\\" + str(self.GlobalTerrain[i])
                        gm_script.write("IMPORT TYPE=AUTO FILENAME=" + chr(34) + DataFile + chr(34) + "\n")
                    break
                OldNumTerrainFiles = NumTerrainFiles

            if COUNTER > 1 and importPathFlag:
                #do i use i or T for the []?
                dataFileOutput.append(str(self.GlobalTerrain[i]))
        ...
        for T in range(len(dataFileOutput)):
            if dataFileOutput[T]:
                DataFile = self.TerrainFolderPath + "\\" + dataFileOutput[T]
                print(DataFile)
                maps_df.write(DataFile + "\n")
...
test.setFolderPath("A:\Spatial Datalyst\SCRIPT\Step 2 Script A\ExampleS2A - after 1 AllPathA")
#TerrainPath = ex.openFolderNameDialog("Find Terrain Folder")
test.setTerrainDataPath("E:\Terrain")
...
print("1 for USGS GTOPO30 TERRAIN DATABASE FOR THE WORLD (1 KM GRID)")
print("2 for USGS NATIONAL ELEVATION DATABASE FOR THE US (30 METER NED)")
print("3 for USGS PUERTO RICO and THE US VIRGIN ISLANDS (30 METER DEM)")
print("4 for SHUTTLE TERRAIN DATA FOR THE WORLD (90 METER SRTM)")
print("5 for SHUTTLE TERRAIN DATA FOR THE US (30 METER SRTM)")
print("6 for CANADA CDED 1: 50,000 SCALE TERRAIN DATA FILES (10 - 20 METER)")
print("7 for CANADA CDED 1:250,000 SCALE TERRAIN DATA FILES (30 - 90 METER)")
print("8 for USGS NATIONAL ELEVATION DATABASE FOR THE US (10 METER NED)")
# option = int(input())
        
#terrOption = input("Enter the Terrain Option (1-8): ")
test.setTerrainOption(1)
#LULCOption = input("Enter the Land Use Land Clutter (y or n): ")
test.setLULC("y")
#retainOption = input("Enter the Retain Index (y or n): ")
test.setRetainIndex("y")
