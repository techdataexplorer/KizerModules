...
def subroutine6500(Gfreq, RainProb):
    C0 = 0.12
    if Gfreq >= 10:
...
def calculateMultipathOutage(TempF, ClimateFactor, PathRoughness, Freq, Distance, FadeMargin, DFM, RelDispersion, AntennaGain, SDGain, DivSpacing):
...
def calculateMultipathOutageITUR(Site1Latitude, Site2Latitude, Distance, dN1, Sa, HL, Gfreq, FadeMargin, LeftAntHt, RightAntHt):
...
FilePath = ExampleS3FolderPath + "/Data/PathDataPass3.csv"
...
RRCurvesFolderPath = ExampleS3FolderPath + "/Data/RRCurves"
...
for index, row in path_data_pass_df_2.iterrows():
    ...
    SecFFyear, SecDFyear, TotalSecYear = calculateMultipathOutage(TempF, ClimateFactor, PathRoughness, Freq, Distance, FadeMargin, DFM, RelDispersion, AntennaGain, SDGain, DivSpacing) # 'calculate multipath outage using Vigants-Barnett method
    ...
    for index, row in rain_city_index_df.iterrows():
        ...
        Zmiles = distance_between_a_and_b("m", LATITUDEA, LatitudeB, LONGITUDEA, LongitudeB)
    ...
    SDGain = LeftSDAntennaGain
    DivSpacing = LeftSDSpacing
    SecFFyear, SecDFyear, TotalSecYear = calculateMultipathOutageITUR(Site1Latitude, Site2Latitude, Distance, dN1, Sa, HL, Gfreq, FadeMargin, LeftAntHt, RightAntHt)
    RightLeftFFSecs = SecFFyear
    RightLeftDFSecs = SecDFyear
    RightLeftTotalSecs = TotalSecYear
    ...
    while True:
        RainProb = 0.001
        Ap = subroutine6500(Gfreq, RainProb)
        if Ap < M:
