...
#110
def makeFiles(FolderPath, Answers,LandUse,MaxDataPointsN,DistanceFraction): #110
    ...
    PathFreqFileName = FolderPath + "\TempFile\\" + "PathFreq.CSV" #66
    PathDataFileName = FolderPath + "\TempFile\\" + "PathData.CSV" #67  
    PathFreqFile = open(PathFreqFileName, "w")
    PathDataFile = open(PathDataFileName, "w")
    PathFileName = FolderPath + "\\" + DataFile + ".CSV"
    ...
    for index1, pfRow1 in PathFile_df.iterrows():
        ...
        TempFilePName = FolderPath + "\TempFile\P" + str(ProfileNumber) + ".CSV" 
        TempFilePAName = FolderPath + "\TempFile\PA" + str(ProfileNumber) + ".CSV" 
        ...
        for index4, pfRow4 in TempFileP_df.iterrows():
            ...
            if(LandUse == "Y"):
                LANDUSECODE = int(pfRow4[3])
                LUcode, LANDUSECODESTR = landUseConv(LANDUSECODE)
            if(LUcode == "B"):
                ObstructionHeight = str(BuildingHt) 
            if(LUcode == "T"):
                ObstructionHeight = str(TreeHt)     
            
            Zkm, Zmiles = calcDistance(LATITUDEA,LATITUDEB,LONGITUDEA,LONGITUDEB)        
        ...
        if(CompChoice == 1):
            CompressionRatio = 1
        if(CompChoice == 2):
            CompressionRatio = int((float(DistanceFraction) / float(DistanceIncrement) + .5))
        if(CompChoice == 3):
            CompressionRatio = ProfileCount // int(MaxDataPointsN)
        ...
        TempFilePAName = FolderPath + "\TempFile\PA" + ProfileNumber + ".CSV" 
        TempFilePBName = FolderPath + "\TempFile\PB" + ProfileNumber + ".CSV" 
        ...
        for index6, pfRow6 in TempFilePA_df.iterrows():
            ...
            findTerrain(Site1Elevation,TwrHt1,Site2Elevation,TwrHt2,Site2Distance,distance,MaxELEVATION,SiteHeight4MinInf,SiteHeight4Min43,SiteHeight4Min1,SiteHeight4Min23,SiteHeight4Min12,SiteHeight4Min512,SiteHeight4Min410)     
        ...
        TempFilePBName = FolderPath + "\TempFile\PB" + ProfileNumber + ".CSV" 
        TempFilePCName = FolderPath + "\TempFile\PC" + ProfileNumber + ".CSV" 
        TempFilePB_df = pd.read_csv(TempFilePBName, header = None) #11
        TempFilePC = open(TempFilePCName, "w")    #12
        print("     Final Processing    - PC")

        if(LandUse == "Y"): #No further processing required - terrain obstructuions are already added
        ...
        if(LandUse == "N"): #Add trees at peaks and locations closest to path
...
#4000
def findTerrain(Site1Elevation,TwrHt1,Site2Elevation,TwrHt2,Site2Distance,distance,MaxELEVATION,SiteHeight4MinInf,SiteHeight4Min43,SiteHeight4Min1,SiteHeight4Min23,SiteHeight4Min12,SiteHeight4Min512,SiteHeight4Min410):
    Site1AntennaElevation = float(Site1Elevation) + float(TwrHt1)
    Site1Distance = 0
    Site2AntennaElevation = float(Site2Elevation) + float(TwrHt2)
    Site2Distance = float(Site2Distance)
...
#6000
def calcDistance(LATITUDEA,LATITUDEB,LONGITUDEA,LONGITUDEB): #CALCULATE DISTANCE BETWEEN SITE A AND SITE B
...
def optimizeTower(TwrHtFlag,START1,END1,START2,END2,FolderPath,ProfileNumber,Site1Elevation,Site2Elevation,TwrHt1,TwrHt2,Site2Distance,SecondKFactor,ITURkFactor,ALUkFactor,FirstKFactor,DLaneTest,FirstFresnelFraction,SecondFresnelFraction,PathEval,WorstCaseObst,OpFreq,AddlClearance,Test2Flag):
    ...
    for II in range(IIEND):
        ...
        FileName = FolderPath + "\TempFile\PC" + ProfileNumber + ".CSV" #11
        ...
        #957
            ...
            if(EndOfFile):
                ...
                Eval = 2
                evalPoint(distance,ELEVATION,ObstructionHeight,Eval,Site1Elevation,Site2Elevation,TwrHt1,TwrHt2,AntHt1,AntHt2,Site2Distance,SecondKFactor,ITURkFactor,FeetMeters,ALUkFactor,FirstKFactor,DLaneTest,FirstFresnelFraction,SecondFresnelFraction,PathEval,WorstCaseObst,OpFreq,AddlClearance,Test2Flag)
...
#9000
def initializeSubroutine(Criteria): 
...
print("Path Evaluation Program")
print("\n(C) TeleVision, Inc.")
print("georgekizer@gmail.com")
print("972.333.0712 / 972.618.2890\n") 

FolderPath = ex.openFolderNameDialog("Enter name of folder(s) containing the input (e.g., ProfData)")
...
Criteria = (FolderPath + "\Criteria.ini")
Answers = initializeSubroutine(Criteria) #9000
...
bool = False
while(not bool):
    print("Pick one of three profile compression choices:")
    print("1 - No compression")
    print("2 - Compress each path to a nominal incremental distance")
    print("3 - Compress each path to a nominal number of samples")
    CompChoice = int(input("Enter your choice (1, 2 or 3)\n"))
...
if(CompChoice == 1):
    MaxDataPointsN = 1
    DistanceFraction = None
    makeFiles(FolderPath,Answers,LandUse,MaxDataPointsN,DistanceFraction)

if(CompChoice == 2):
    MaxDataPointsN = 1
    DistanceFraction = input("\nEnter approximate fraction of a mile or kilometer for path increments\n")
    makeFiles(FolderPath,Answers,LandUse,MaxDataPointsN,DistanceFraction)

if(CompChoice == 3):
    DistanceFraction = None
    print("\nEnter approximate maximum number of path profile data points (N).")
    MaxDataPointsN = input("The final profile number of points will be between N and 2N ")
    makeFiles(FolderPath,Answers,LandUse,MaxDataPointsN,DistanceFraction)
