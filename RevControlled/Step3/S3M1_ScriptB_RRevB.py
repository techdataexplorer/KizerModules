...
class ScriptB(object):
    ...
    def AddGlobalFile(self, FirstFlag, Latitude, Longitude, USGSindxFilePath, USGS250kFilePath):
        if self.LandUseFlag == 0:
            ...
            if self.NewGlobalFile[0] != "":                                                          #USned10m/n34w097/grdn34w097_13/W001001.adf (non null equal true)
                #NumTerrainFiles = self.MaxGlobal                                               # why 101 (arbitrary)
        ...
        #FIND USGS LANDUSE (LULC) FILE
            ...
            df = pd.read_csv(Path(USGSindxFilePath))
            ...
            df = pd.read_csv(Path(USGS250kFilePath), header=None)
    ...
    def execute(self):
        ...
        InputFile = Path(self.ExampleFolder + "/Paths.CSV")


        # Use Terrain folder path to create USGSindx.csv file path
        USGSindxFilePath = Path(self.TerrainDir + "/LandUse/Data/USGSindx.csv")

        # Use Terrain folder path to create USGS250k.csv file path
        USGS250kFilePath = Path(self.TerrainDir + "/LandUse/Data/USGS250k.csv")
        ...
        globalMapperScript = Path(self.ExampleFolder + "/GMScript.gms")
        geonadFile = Path(self.TerrainDir + "/Misc/GEO_NAD83.prj")
        if self.option == 4 or self.option == 7 or self.option == 8:
            geonadFile = Path(self.TerrainDir + "/Misc/GEO_WGS84.prj")

        OutputFile = open(globalMapperScript,"w+")
        OutputFile.write("GLOBAL_MAPPER_SCRIPT VERSION=1.00\n")
        OutputFile.write("UNLOAD_ALL\n")
        OutputFile.write("LOAD_PROJECTION FILENAME=" + str(geonadFile) + "\n")
        ...
        for index, row in df.iterrows():
            ...
            if COUNTER == 1:
                for i in range(1,self.MaxGlobal):
                    if  self.GlobalTerrain[i] != "":
                        DataFile = Path(str(self.TerrainDir) + "/" + str(self.GlobalTerrain[i]) + "\n")
                        OutputFile.write("IMPORT TYPE=AUTO\n" + "FILENAME=" + str(DataFile))
            ...
            if COUNTER > 1 and NumTerrainFiles > OldNumTerrainFiles:
                for T in range(OldNumTerrainFiles + 1, self.MaxGlobal):
                    if  self.GlobalTerrain[T] != "":
                        DataFile = Path(str(self.TerrainDir) + "/" + str(self.GlobalTerrain[T]) + "\n")
                        OutputFile.write("IMPORT TYPE=AUTO\n" + "FILENAME=" + str(DataFile))
            ...
            DataFile = DataFile + " START_POS=" + str(LONGITUDEA) + "," + str(LATITUDEA) + " END_POS=" + str(LONGITUDEB) + "," + str(LATITUDEB)

            OutputFile.write(DataFile)

        maps_df = open(Path(self.ExampleFolder + "/Maps.csv"), "w+")
        maps_df.write(str(ProjectionFile) + "\n")
...
ExampleS3FolderPath = ex.openFolderNameDialog("Find ExampleStep3 Folder")
