...
class ParameterB(object):
    ...
    def execute(self):
        ...
        for index, row in average_paths_df.iterrows():
            ...
            path_parameters_df.write(str(PathIndex) + "," + str(PathDistance[PathIndex]) + "," + str(self.TheRoughness[PathIndex]) + "," + str(int(TheClimateFactor[PathIndex])) + "," + str(TempF[PathIndex]) + "," + str(dN1[PathIndex]) + "," + str(Sa[PathIndex]) + "," + str(R01[PathIndex]) + "," + str(int(RelHum[PathIndex])) + "," + str(self.TheHeight[PathIndex])  + '\n') # added  + '\n'
