...
class ScriptB(object):
    ...
    def __init__(self):
        ...
        self.TerrainDir = ""
        # Get option
        self.option = float("inf")     # user input
    ...
    def FindWorldSrtm(self, Latitude, Longitude):
        ...
        for T in range(SrtmMax):
            ...
            if self.Srtm[T][1] <= Latitude and Latitude <= self.Srtm[T][2] and self.Srtm[T][3] <= Longitude and Longitude <= self.Srtm[T][4]:
    ...
    def FindUsSrtm(self, Latitude, Longitude):
        ...
        for T in range(SrtmMax):
            #check this line with qbasic
            #this should go in the if statment during counter1 and counter2
            #if self.Srtm[T][0] <= Latitude and Latitude <= self.Srtm[T][1] and self.Srtm[T][2] <= Longitude and Longitude <= self.Srtm[T][3]:
            #if self.Srtm[T][2] <= Latitude and Latitude <= self.Srtm[T][3] and self.Srtm[T][4] <= Longitude and Longitude <= self.Srtm[T][5]:
            if self.Srtm[T][1] <= Latitude and Latitude <= self.Srtm[T][2] and self.Srtm[T][3] <= Longitude and Longitude <= self.Srtm[T][4]:
    ...
    def AddGlobalFile(self, FirstFlag, Latitude, Longitude, USGSindxFilePath, USGS250kFilePath):
        if self.LandUseFlag == 0:
            ...
            if self.NewGlobalFile[0]:
                #self.NumTerrainFiles = self.MaxGlobal
                for T in range(self.MaxGlobal):
                    self.GlobalTerrain[T] = None
                for T in range(self.MaxGlobal):
                    if (self.GlobalTerrain[T] == None):
                        self.GlobalTerrain[T] = self.NewGlobalFile[0]
                        self.NumTerrainFiles = T
                    if self.GlobalTerrain[T] == self.NewGlobalFile[0]:
                        if self.NumTerrainFiles > (self.MaxGlobal):
    ...
    def execute(self):
        ...
        globalMapperScript = self.ExampleFolder + "\\GMScript.gms" 
        ...
        for index, row in df.iterrows():
            ...
            mapStr = str(self.GlobalTerrain[0])            
            mapArray.append(mapStr)

            if mapStr == 'None':
                importPathFlag = False
            elif mapStr in dataFileOutput:
                importPathFlag = False
            else:
                importPathFlag=True


            if COUNTER == 1 and importPathFlag:
                for T in range(self.MaxGlobal):
                    if (self.GlobalTerrain[T] != None):
                        dataFileOutput.append(str(self.GlobalTerrain[T]))
                        DataFile = str(self.TerrainDir) + "\\" + str(self.GlobalTerrain[T])
                        OutputFile.write("\n" + "IMPORT TYPE=AUTO FILENAME=" + "\"" + DataFile + "\"")
                    break
                OldNumTerrainFiles = self.NumTerrainFiles

            #so the 2nd import type line has to be in this one, 
            #the issue is in self.NumTerrainFiles > OldNumTerrainFiles:
            #oldnumterrain doesnt change

            #if globalterrain[0] is not in datafile array, output
            # if mapStr == 'None':
            #     importPathFlag = False
            # elif mapStr in dataFileOutput:
            #     importPathFlag = False
            ...
            #if COUNTER > 1 and self.NumTerrainFiles > OldNumTerrainFiles:
            if COUNTER > 1 and importPathFlag:
                dataFileOutput.append(str(self.GlobalTerrain[T]))
            ...
            #maybe we make a new array
            #have global terrain go into that array


            #if any duplicates, ignore those
            #so now this array should only the 2 strings for maps.csv
            #use that array in the for loop for maps.csv
        ...
        #PRINTS LINES FOR MAP.CSV

        #MAKE TEST CASE FOR NONE
        for T in range(len(dataFileOutput)):
            if dataFileOutput[T]:
                DataFile = self.TerrainDir + "\\" + dataFileOutput[T]
                maps_df.write(DataFile + "\n")
...
test = ScriptB()
#test.setFolderPath("A:\Spatial Datalyst\Kizer\Path Design 11 April 2021\Step 3 Path Availability\ExampleStep3")
ExampleS3FolderPath = "A:\Spatial Datalyst\SCRIPT\Step 3 Script B\ExS3"
test.setFolderPath(ExampleS3FolderPath)
#test.setTerrainFolder("E:\Terrain")
TerrainFolderPath = "E:\Terrain"
test.setTerrainFolder(TerrainFolderPath)
# test.setOption(4)
...
userOption = input("Enter Terrain Data File Number: ")
test.setOption(userOption)
test.execute()
input("Press Enter to exit")

#get that 1 line added
#use pathlib for georges side
#make into exe
#make sure exe works on our end
#submit to george
