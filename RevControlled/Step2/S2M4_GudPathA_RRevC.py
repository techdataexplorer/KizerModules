...
class GudPath(object):
    ...
    def iniFileReader(self, iniFilePath): #9000
        ...
        fileData = []
        inputFile = open(Path(iniFilePath), "r")
        counter = 0
    ...
    def optimize_one_tower(self, ShortestWGrun):
        ...
        for II in range(self.IISTART, self.int_IIEND):
            ...
            profile_num_df = pd.read_csv(Path(self.ProfileNumberFilePath), header=None)       #11
    ...
    def execute(self):
        ...
        CriteriaFilePath = Path(self.ExampleS2AFolderPath + "/Criteria.csv")
        ...
        PassStatFilePath = Path(self.ExampleS2AFolderPath + "/Passed/PassStat.csv")        #51
        pass_stat_df = open(PassStatFilePath, "w+")

        FailStatFilePath = Path(self.ExampleS2AFolderPath + "/Failed/FailStat.csv")        #52
        fail_stat_df = open(FailStatFilePath, "w+")

        PassSiteFilePath = Path(self.ExampleS2AFolderPath + "/Passed/PassSite.csv")        #56
        pass_site_df = open(PassSiteFilePath, "w+")

        PassPathFilePath = Path(self.ExampleS2AFolderPath + "/Passed/PassPath.csv")        #57
        pass_path_df = open(PassPathFilePath, "w+")

        FailSiteFilePath = Path(self.ExampleS2AFolderPath + "/Failed/FailSite.csv")        #58
        fail_site_df = open(FailSiteFilePath, "w+")

        FailPathFilePath = Path(self.ExampleS2AFolderPath + "/Failed/FailPath.csv")        #59
        ...
        FinalPathData = Path(self.ExampleS2AFolderPath + "/Optimize/PathInfo.csv")
        ...
        PassedPathData = Path(self.ExampleS2AFolderPath + "/Passed/PathInfo.csv")
        passed_path_data_df = open(PassedPathData, "w+")        #16

        # Open the file with the path definition data
        PathFileFilePath = Path(self.ExampleS2AFolderPath + "/" + TheDataFile + ".csv")
        ...
        for index, row in data_file_df.iterrows():    # Walk through the path definitions
            ...
            PathFreqFilePath = Path(self.ExampleS2AFolderPath + "/TempFile/PathFreq.csv")
            ...
            # Determine path data
            PathDataFilePath = Path(self.ExampleS2AFolderPath + "/TempFile/PathData.csv")
            ...
            self.ProfileNumberFilePath = Path(self.ExampleS2AFolderPath + "/TempFile/PC" + str(self.ProfileNumber) + ".csv")
            ...
            if FailFlag == 0:
                ...
                for k, profile in pc_df.iterrows():
                    ...
                    EVDuplicatePath = Path(self.ExampleS2AFolderPath + "/Failed/EV" + str(self.ProfileNumber) + ".csv")
                    PRDuplicatePath = Path(self.ExampleS2AFolderPath + "/Failed/PR" + str(self.ProfileNumber) + ".csv")
        ...
        StatusFilePath = Path(self.ExampleS2AFolderPath + "/TempFile/Status.csv")      #57
        ...
        PassSiteFilePath2 = Path(self.ExampleS2AFolderPath + "/Passed/PassSite.csv")
        pass_site_df = pd.read_csv(PassSiteFilePath2, header=None)       #56

        Temp1FilePath = Path(self.ExampleS2AFolderPath + "/TempFile/Temp1.csv")
        ...
        pass_site_df = open(Path(PassSiteFilePath2), "w")              #57
        pass_site_df.close()

        temp1_df.close()
        temp1_df = pd.read_csv(Path(Temp1FilePath), header=None)       #56
        ...
        for i, row in temp1_df.iterrows():
            ...
            if i != 0:  # dont try to read from this file on first pass (its empty at first)
                pass_site_df = pd.read_csv(Path(PassSiteFilePath2), header=None)      #57
            ...
            if Flag == 0:
                pass_site_df = open(Path(PassSiteFilePath2), "a")      #57
                pass_site_df.write(str(self.SITE1) + "," + str(Latitude1) + "," + str(Longitude1) + "\n")
                pass_site_df.flush()
                pass_site_df.close()
        ...
        FailSiteFilePath2 = Path(self.ExampleS2AFolderPath + "/Failed/FailSite.csv")

        try:
            failed_site_df = pd.read_csv(FailSiteFilePath2, header=None)       #56
        
            Temp1FilePath2 = Path(self.ExampleS2AFolderPath + "/TempFile/Temp1.csv")
        ...
        InputFileSites = Path(self.ExampleS2AFolderPath + "/Passed/PassSite.csv")
        InputFilePaths = Path(self.ExampleS2AFolderPath + "/Passed/PassPath.csv")
        OutputFileGoogle = Path(self.ExampleS2AFolderPath + "/Passed/Google/GEPaths.KML")
        # Create Maps
        self.createKML(InputFileSites, InputFilePaths, OutputFileGoogle)

        # Failed sites and paths
        InputFileSites = Path(self.ExampleS2AFolderPath + "/Failed/FailSite.csv")
        InputFilePaths = Path(self.ExampleS2AFolderPath + "/Failed/FailPath.csv")
        OutputFileGoogle = Path(self.ExampleS2AFolderPath + "/Failed/Google/GEPaths.KML")
        # Create Maps
        try:
            self.createKML(InputFileSites, InputFilePaths, OutputFileGoogle)
        except pd.errors.EmptyDataError:
